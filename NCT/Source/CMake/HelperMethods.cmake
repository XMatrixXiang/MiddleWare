...
function(make_ftp_bat_and_exc WORK_DIR DOWNLOAD_FILE_PATH)
	SET(FTP_BAT_PATH "${WORK_DIR}/ftp_thirdParty.bat")
	SET(FTP_BAT_EXC_PAR_PATH "${WORK_DIR}/thirdParty.bat")
	SET(FTP_DOWNLOAD_RESULT "")
	file(WRITE ${FTP_BAT_PATH} "@echo off\nftp -s:${WORK_DIR}/thirdParty.bat")
	file(WRITE ${FTP_BAT_EXC_PAR_PATH} "open 10.0.0.10\nruanjianbu\n12345678\nprompt off\nlcd ${WORK_DIR}\nget ${DOWNLOAD_FILE_PATH}\nbye")
	message(STATUS "begin download!")
	execute_process(COMMAND "${FTP_BAT_PATH}" WORKING_DIRECTORY ${WORK_DIR}  OUTPUT_VARIABLE FTP_DOWNLOAD_RESULT)
	message(STATUS ${FTP_DOWNLOAD_RESULT})
	message(STATUS "end download!")		
	file(REMOVE ${FTP_BAT_PATH})
	file(REMOVE ${FTP_BAT_EXC_PAR_PATH})
endfunction()
...
function(update_binary_deps DEP_PREFIX DEP_FOLDER DEP_VERSION)
	...
	#endif()
	make_ftp_bat_and_exc("${PROJECT_SOURCE_DIR}/Temp" "SimTools/ShipEditor/${DEP_PREFIX}Dependencies_${DEP_TYPE}_Master_${DEP_VERSION}.zip" )
	message(STATUS "Extracting files. Please wait...")
	execute_process(
		COMMAND ${CMAKE_COMMAND} -E tar xzf ${PROJECT_SOURCE_DIR}/Temp/${DEP_PREFIX}Dependencies_${DEP_TYPE}_Master_${DEP_VERSION}.zip
...
function(update_builtin_assets ASSET_PREFIX ASSET_FOLDER ASSET_VERSION CLEAR_MANIFEST)
	...
	#endif()
	make_ftp_bat_and_exc("${PROJECT_SOURCE_DIR}/Temp" "SimTools/ShipEditor/${ASSET_PREFIX}Data_Master_${ASSET_VERSION}.zip" )
	message(STATUS "Extracting files. Please wait...")
	execute_process(
		COMMAND ${CMAKE_COMMAND} -E tar xzf ${PROJECT_SOURCE_DIR}/Temp/${ASSET_PREFIX}Data_Master_${ASSET_VERSION}.zip
		WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}/Temp
	)
